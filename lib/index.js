const fs = require("fs");
// const {is} = require('ssh2-connect')

const isOpen = (ssh) =>
  // ssh@0.3.x use "_state"
  // ssh@0.4.x use "_sshstream" && "_sock"
  // ssh@1.7.0 use "ssh._writableState?.ended"
  (ssh._state ?? ssh._state !== "closed") ||
  (ssh._sshstream?.writable && ssh._sock?.writable) ||
  ssh._sock?._writableState?.ended === false;
...
module.exports = {
  ...
  */
  chmod: async function (ssh, path, mode) {
    if (!ssh) {
      return fs.promises.chmod(path, mode);
    } else {
      return new Promise((resolve, reject) => {
        if (!isOpen(ssh)) return reject(Error("Closed SSH Connection"));
        ssh.sftp((err, sftp) => {
  ...
  */
  ...
  mkdir: async (ssh, path, options = 0o0777) => {
