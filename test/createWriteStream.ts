...
describe("createWriteStream", function () {
  ...
  describe("error", function () {
    they("ENOENT if parent dir does not exists", async ({ ssh }) => {
      await new Promise((resolve, reject) => {
        ssh2fs
          .createWriteStream(ssh, `${scratch}/not/here`, "utf8")
          .then((stream) => {
    ...
    they("EISDIR if file is a directory", async ({ ssh }) => {
      await new Promise((resolve, reject) => {
  ...
  they("pipe into stream reader", async ({ ssh }) => {
